...
def flame_length_measure(img, threshold):
    ...
    # for j in range(0, img_theshold.shape[0]-1, 1):
    #     for i in range(0, img_theshold.shape[1]-1, 1):
    #         if ((img_theshold[j,i] < 60) or (img_theshold[j,i]>140)):
    #             img_theshold[j,i] = 0
            
    # cv.imshow('assd', img_theshold)
    img_theshold = cv.Canny(img_theshold,20, 170)
    # cv.imshow('assd', edges)
...
# for the trailing edge
    # for k in range(canny1.shape[1]-1, 0, -1):
    #     if (sum(canny1[:,k])/canny1.shape[0] > threshold):
...
# for the leading edge
    for k in range(0, img_theshold.shape[1]-1, 1):
...
k = flame_length_measure('3583-S4C3.jpg', 50)