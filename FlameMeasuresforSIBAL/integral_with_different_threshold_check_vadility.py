...
def flame_length_measure(img):
    ...
    img_theshold = cv.Canny(img_theshold,30,200)
    ...
    ax.set_xlabel('Width(pixles)', fontsize = 16)
    ax.set_ylabel('Length(pixles)', fontsize = 16)
    ax2 = ax.twinx()
    ax2.plot(inte_col, color='r', linewidth=5)
    ax2.axhline(y=30, xmin=0.05, xmax=1, linestyle = "--", color='r')
    ax2.axvline(x=k_30, ymin=0, ymax=1, linestyle = "--", color='yellow', linewidth=3)
    ax2.set_ylabel('Averaged intensity', fontsize = 16)
    ax2.tick_params(axis = 'y', labelsize=16)
    # ax.set_title('threshold 30',fontsize=24,color='k')
...
k = flame_length_measure('S4C31431.jpg')